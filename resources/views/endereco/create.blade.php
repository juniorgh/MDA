...
@section('content')
    {{-- <x-colaborador.create-edit :user_id="$user_id" :profissoes="$profissoes" /> --}}

    @if(!empty($endereco->id))
    <form class="crud-create-form" action="{{ route('endereco.update',['id' => $endereco->id ]) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
@else
